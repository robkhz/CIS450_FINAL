...
PDM_SOUND_TYPE get_sound_by_light_level(uint8_t lightLevel)
{
    switch (lightLevel)
    {
    case 0:
        return SOUND_TYPE_LIGHT_LEVEL_0;
    case 25:
        return SOUND_TYPE_LIGHT_LEVEL_25;
    case 50:
        return SOUND_TYPE_LIGHT_LEVEL_50;
    case 75:
        return SOUND_TYPE_LIGHT_LEVEL_75;
    case 100:
        return SOUND_TYPE_LIGHT_LEVEL_100;
    default:
        return SOUND_TYPE_FACTORY;
    }
}
        ...
        if (is_time_out(&time_500ms))
        {
            if (LV_KEY_RIGHT == key)
            {
                if (light_set_conf.light_pwm < 100)
                {
                    ...
                    PDM_SOUND_TYPE sound = get_sound_by_light_level(light_set_conf.light_pwm);
                    audio_handle_info(sound);
                }
            }
